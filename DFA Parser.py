...
with open(sys.argv[1]) as f:
    ...
    while index < len(lines):
        ...
        if lines[index][0] != '#':
            if 'Sigma' == ''.join(spl('[:\s]+', lines[index])):
                index += 1
                while index < len(lines) and 'End' != lines[index].strip():
                    if lines[index][0] == '#' or not (lines[index].strip()):
                        print('Format error: Section "Sigma" must end with "End" line.')
                        quit()
                    if not (lines[index].strip().isalnum()):
            ...
            elif 'States' == ''.join(spl('[:\s]+', lines[index])):
                index += 1
                while index < len(lines) and 'End' != lines[index].strip():
                    if lines[index][0] == '#' or not (lines[index].strip()):
                    ...
                    if ',S' in lines[index]:
                        if S != '':
                            print(
                                'Condition error: There can only be one initial state.')
                            quit()
                        else:
                            S = state
                    index += 1
            ...
            elif 'Transitions' == ''.join(spl('[:\s]+', lines[index])):
                index += 1
                while index < len(lines) and 'End' != lines[index].strip():
                    if lines[index][0] == '#' or not (lines[index].strip()):
