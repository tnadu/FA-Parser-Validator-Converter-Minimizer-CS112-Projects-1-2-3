...
if Accept==True:
    ...
    # each letter of the given word is verified

        stop = True
        # we assume that the word would not pass the verification test

        for j in range(len(states)):
            if T[current_state][j] == word[i]:
                current_state = j
                # we continue the verification process since the current letter was correct
                stop = False #hence there is no reason to stop
                break
    ...
    #print(chr(current_state))
    #print(F)
    if stop == False and current_state in [int(x) for x in F]:
    # finally, we check that the final state is a member within F
        print(f"The word {word} was accepted!")
    else:
